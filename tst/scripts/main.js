...
function removeNode(){
	alert("not all nodes are removed if multiple selected");
	if(removingNode == 0){
		if(addingConnecters == 0){
			removingNode = 1;
			alert("Click on nodes you wish to remove and then click this button again when finished.");
...
function addConnecters(){
	if(addingConnecters == 0){
		...
		for(var i = 0; i < newPairs.length; ++i){
			var newConnecter = $('<div class="connecter" id="connecter_' + newPairs[i][0] + newPairs[i][1] + '"></div');
